...
@Composable
fun SearchScreen(cartViewModel: CartViewModel, listingViewModel: ListingViewModel = viewModel()) {
    var query by remember { mutableStateOf("") }
    val allListings by listingViewModel.allListings.collectAsState()

    val results = if (query.isEmpty()) {
        allListings
    } else {
        allListings.filter { it.listingName.contains(query, ignoreCase = true) }
    }
